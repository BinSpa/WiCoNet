...
def main():
    net = Net(4, num_classes=FBP.num_classes + 1, size_context=args['size_context'],
              size_local=args['size_local']).cuda()

    train_set = FBP.Loader(args['data_dir'], 'train', random_crop=True, crop_nums=args['crop_nums'], random_flip=True,
...
def validate(val_loader, net, criterion, curr_epoch):
    ...
    for vi, data in enumerate(val_loader):
        ...
        if args['save_pred'] and vi == 0:
            pred_color = FBP.Index2Color(preds[0])
