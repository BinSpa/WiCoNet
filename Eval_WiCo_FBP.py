...
##################################################
from datasets import FBP_WiCo as FBP
...
args = {
    ...
    'load_path': '/data1/gyl/RS_Code/WiCoNet/checkpoints/FBP/WiCoNet_3hw4L_47e_OA85.90.pth'
}
...
def main():
    net = Net(4, num_classes=FBP.num_classes + 1, size_context=args['size_context'], size_local=args['size_local']).cuda()
...
def predict(net, pred_loader, pred_path, f_out=None):
    ...
    for vi, data in enumerate(pred_loader):
        ...
        pred_color = FBP.Index2Color(pred)
        ...
        TP, pred_hist, label_hist, union_hist = CaclTP(pred, label, FBP.num_classes)
        TP_meter.update(TP)
