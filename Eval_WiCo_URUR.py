...
args = {
    ...
    'load_path': '/data1/gyl/RS_Code/WiCoNet/checkpoints/URUR/WiCoNet_3hw4L_27e_OA80.78.pth'
}
...
def main():
    net = Net(3, num_classes=URUR.num_classes + 1, size_context=args['size_context'], size_local=args['size_local']).cuda()
    ...
    val_set = URUR.Loader(args['data_dir'], 'test', sliding_crop=True, size_context=args['size_context'], size_local=args['size_local'])
...
def predict(net, pred_loader, pred_path, f_out=None):
    ...
    for vi, data in enumerate(pred_loader):
        ...
        pred_color = URUR.Index2Color(pred)
        ...
        TP, pred_hist, label_hist, union_hist = CaclTP(pred, label, URUR.num_classes)
        TP_meter.update(TP)
