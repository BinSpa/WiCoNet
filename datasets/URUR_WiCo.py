...
for i, cm in enumerate(URUR_COLORMAP):
    colormap2label[(cm[0] * 256 + cm[1]) * 256 + cm[2]] = i


def Index2Color(pred):
    colormap = np.asarray(URUR_COLORMAP, dtype='uint8')
...
def read_RSimages(data_dir, mode, rescale_ratio=False):
    ...
    for it in data_list:
        ...
        label = np.array(Image.open(mask_path)).astype(np.uint8)
