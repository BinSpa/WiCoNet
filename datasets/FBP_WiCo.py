...
FBP_COLORMAP = [[0, 0, 0], [200,0,0], [0,200,0], [150,250,0], [150,200,150], 
...
FBP_CLASSES = ['unlabeled', 'industrial area', 'paddy field', 'irrigated field', 'dry cropland',
...
FBP_MEAN_IRRGB = np.array([126.72, 90.76, 95.96, 87.58])
FBP_STD_IRRGB = np.array([62.91, 59.24, 58.03, 57.23])
FBP_MEAN_RGB = np.array([90.76, 95.96, 87.58])
FBP_STD_RGB = np.array([59.24, 58.03, 57.23])
...
def normalize_image(im):
    return (im - FBP_MEAN_IRRGB) / FBP_STD_IRRGB
...
for i, cm in enumerate(FBP_COLORMAP):
    colormap2label[(cm[0] * 256 + cm[1]) * 256 + cm[2]] = i

def Index2Color(pred):
    colormap = np.asarray(FBP_COLORMAP, dtype='uint8')
...
def read_RSimages(data_dir, mode, rescale_ratio=False):
    ...
    for it in data_list:
        img_path = it
        mask_path = img_path.replace('images', 'rgb_fbp_labels') 
        mask_path = mask_path.replace('.tif', '_24label.png')  
